// frontend/src/pages/ThiDuaHS/modals/ModalThemVPCaNhan.jsx
...
export default function ModalThemVPCaNhan({
  open, onClose, onSuccess, dsHS, dsLoaiVPCaNhan, selNamHoc, tuan, form
}) {
  const handleSubmit = async () => {
    try {
      ...
      await caNhanAPI.create({
        hoc_sinh_id: v.hoc_sinh_id,
      ...
      form.resetFields()
      await onSuccess(v)  // ⭐ Đảm bảo await và truyền dữ liệu
      onClose()
    } catch (err) {
      ...
      <Form form={form} layout="vertical" style={{ marginTop: 12 }}>
        ...
        <Form.Item name="loai_vi_pham_id" label="Loại vi phạm / thành tích" rules={[{ required: true }]}>
          <Select
            ...
            onChange={(value) => {
              const selected = dsLoaiVPCaNhan.find(l => l.id === value)
              if (selected) form.setFieldsValue({ so_diem: Math.abs(selected.so_diem) })
            }}
        ...
        <Form.Item name="so_diem" label="Điểm số" rules={[{ required: true }]} initialValue={0}>
