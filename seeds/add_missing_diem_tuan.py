# seeds/add_missing_diem_tuan.py
import sqlite3
import os
import random
...
try:
    ...
    # Lấy danh sách lớp và kiểm tra xem đã có dữ liệu chưa
    cursor.execute("SELECT id, ten_lop FROM lop_hoc ORDER BY id")
    lop_hocs = cursor.fetchall()
    print(f"🏫 Số lớp: {len(lop_hocs)}")

    # Kiểm tra lớp nào đã có dữ liệu
    cursor.execute("SELECT DISTINCT lop_hoc_id FROM diem_tuan")
    existing = [row[0] for row in cursor.fetchall()]
    print(f"📊 Lớp đã có dữ liệu: {existing}")

    # Thêm dữ liệu cho các lớp chưa có
    data = []
    for lop in lop_hocs:
        if lop[0] in existing:
            print(f"⏭️ Lớp {lop[1]} (id={lop[0]}) đã có dữ liệu")
            continue
        for tuan in range(1, 5):  # Tuần 1-4
            trung_binh = round(random.uniform(7.0, 9.5), 2)
            data.append((lop[0], nam_hoc_id, tuan, trung_binh))
            print(f"  - Lớp {lop[1]} (id={lop[0]}), Tuần {tuan}: {trung_binh}")
    ...
    conn.commit()
    print(f"✅ Đã thêm {count} dòng dữ liệu mới vào diem_tuan")
