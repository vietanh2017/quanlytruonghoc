# modules/giao_vien_thi_dua/views/tabs/theo_thang_tab.py
...
class TheoThangTab(QWidget):
    ...
    def __init__(self, svc, nguoi_dung=None, parent=None):
        ...
        # Lưu to_id của người dùng (nếu là giáo viên)
        if nguoi_dung:
            try:
    ...
    def _build_ui(self):
        ...
        # Thêm tháng theo năm học (8->12, 1->5)
        ...
        self.cmb_nam_hoc.currentIndexChanged.connect(self._on_load)

    def _setup_table(self):
        headers = ["STT", "Mã GV", "Họ tên", "Điểm tháng", "Xếp loại", "Xếp thứ"]
    ...
    def _on_load(self):
        ...
        # Lấy danh sách giáo viên theo tổ
        ...
        if not ds_gv:
            ...
            self.summary_label.setText("🏆 TỔNG QUAN: --")
            return
        
        diem_list = []
        for gv in ds_gv:
            r = self.svc.tinh_diem_thang(gv.id, thang, nam_hoc_id)
            if r.ok:
                diem = r.data["diem"]
                xep_loai = self.svc.xep_loai(diem)
                diem_list.append({
                    'gv': gv,
                    'diem': diem,
                    'xep_loai': xep_loai,
        ...
        self.status_label.setText(f"📊 Tháng {thang} - {self.cmb_nam_hoc.currentText()} - {ten_to}")
    ...
    def _update_chart(self, diem_list):
        ...
        if diem_list:
            ...
            thang_hien_tai = self.cmb_thang.currentData()
            tieu_de = f"🏆 Tháng {thang_hien_tai}: "
