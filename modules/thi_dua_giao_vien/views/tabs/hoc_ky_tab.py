# modules/giao_vien_thi_dua/views/tabs/hoc_ky_tab.py
...
class HorizontalBarChartHK(QWidget):
...
class HocKyTab(QWidget):
    ...
    def _build_ui(self):
        ...
        self.cmb_hoc_ky.setMinimumWidth(120)
        ...
        self.bar_chart = HorizontalBarChartHK()
        ...
        self.cmb_nam_hoc.currentIndexChanged.connect(self._load_hoc_ky)

    def _setup_table(self):
        headers = ["STT", "Mã GV", "Họ tên", "Điểm HK", "Xếp loại", "Xếp thứ"]
    ...
    def _load_hoc_ky(self):
        ...
        try:
            ...
            hks = self.svc.session.query(HocKy).filter(HocKy.nam_hoc_id == nam_hoc_id).all()
            self.cmb_hoc_ky.clear()
            for hk in hks:
                self.cmb_hoc_ky.addItem(hk.ten_hoc_ky, hk.id)
            if self.cmb_hoc_ky.count() > 0:
                self.cmb_hoc_ky.setCurrentIndex(0)
        except Exception as e:
            pass
    ...
    def _on_load(self):
        ...
        if not hoc_ky_id:
            QMessageBox.warning(self, "Cảnh báo", "Vui lòng chọn học kỳ!")
            return
        ...
        diem_list = []
        for gv in ds_gv:
            r = self.svc.tinh_diem_hoc_ky(gv.id, hoc_ky_id, nam_hoc_id)
            if r.ok:
                diem = r.data["diem"]
        ...
        self.status_label.setText(f"📚 {ten_hk} - {self.cmb_nam_hoc.currentText()} - {ten_to}")
